...
All paths are relative to `${BASE_URL}`. Unless noted otherwise, operations require `Authorization: Bearer <token>` and return JSON.
...
| POST | `/api/batch/queue` | Submit a batch request to the configured queue; hidden from Swagger |
| GET | `/api/batch/{id}` | Load a batch and its internal data |
...
| PUT | `/api/batch/status/purge/{id}` | Permanently remove a deleted batch |

`POST /api/batch/queue` returns a request ID, not the batch created in Investran. Queue infrastructure, retries, and status queries are not exposed by any controller in this repository.
...
| POST | `/api/investor` | Create an Investor and, optionally, its contact relationship |
...
The two `search/{value}` routes have the same shape and no explicit route constraint. Validate actual routing in the deployed application. In a future API version, prefer an unambiguous route such as `search/name/{name}` or `vehicle/{vehicleId}/investors`.
...
| GET | `/api/position/{name}` | Search Positions whose name contains the supplied text |
...
The integer route constraint distinguishes `/{id}` from `/{name}`.

## Contextual entities (`Specific*`)
...
Most lookup responses use this compact form:
...
Batch lookup responses can also include `active`.

## Security

| Method | Path | Purpose |
|---|---|---|
| GET | `/api/security` | Return security/domain data through the security service |
| GET | `/api/security/{id}` | Load the domain object selected by the controller implementation |

The XML comments and names in `SecurityController` do not accurately describe its implementation. Confirm the response contract before creating a consumer that depends on these endpoints.
